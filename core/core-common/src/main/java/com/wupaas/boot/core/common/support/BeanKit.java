package com.wupaas.boot.core.common.support;

import com.wupaas.boot.core.common.exception.BootException;
import com.wupaas.boot.core.common.util.Convert;
...
public class BeanKit {
    ...
    public static boolean isAllFieldNull(Object obj) {
        ...
        try {
            ...
            List<Field> list = getAllField(obj.getClass(), null);
            flag = true;
            for (Field f : list) {
    ...
    /**
     * 判断该对象是否: 返回True表示所有属性为null,或者空字符串  返回False表示不是所有属性都是null
     * @param obj   对象信息
     * @param excludeParams 排除的属性
     */
    public static boolean isAllFieldNullNoTransient(Object obj, String... excludeParams) {
        boolean flag = false;
        try {
            if (obj == null) {
                return true;
            ...
            for (int i = 0; i < excludeParams.length; i++) {
                excludeSet.add(excludeParams[i]);
            }
            List<Field> list = getAllField(obj.getClass(), null);
            flag = true;
            for (Field f : list) {
                if(!excludeSet.contains(f.getName()) && !Modifier.isTransient(f.getModifiers())){
                    f.setAccessible(true);
                    Object val = f.get(obj);
                ...
                if (sourcePd != null) {
                    ...
                    if (readMethod != null && ClassKit.isAssignable(writeMethod.getParameterTypes()[0], readMethod.getReturnType())) {
                        try {
                            ...
                            throw new BootException(ex, "Copy property [{}] to [{}] error: {}", sourcePd.getName(), targetPd.getName(), ex.getMessage());
                        }
                    }
                }
            }
